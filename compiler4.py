...
def error(num):
    ...
    if num == 1: 
        ...
        print >>outfile, "Const, Var, Procedure, Val, Ref must be followed by an identifier."
    elif num ==5: 
        ...
        print >>outfile, "Function call expected."
    elif num == 28:
    ...
    while sym == "PROCEDURE" or sym == "VAR" or sym == "CONST" or sym == "FUNCTION":
        ...
        if sym == "VAR":
            ...
            if sym != "semicolon":
                error(10)
            getsym()
        while sym == "PROCEDURE" or sym == "FUNCTION": # can be proc or func
            symbol = sym # saves sym for later reference
            getsym()
            if sym == "ident":
                # enters proc or func based on what is needed
                if symbol == "PROCEDURE":
...
def statement(tx, level):
    ...
    if sym == "ident" or sym == "FUNCTION":
        ...
        if i==0:
            ...
            error(12)
        kind = table[i].kind # saves kind
        getsym()
        if sym != "becomes":
            error(13)
        getsym()
        expression(tx, level)
        # does proper action based on "kind"
        if kind == "variable" or kind == "val":
            gen("STO", level-table[i].level, table[i].adr)
        elif kind == "function": # and level > 0: 
            gen("STO", 0, -1)
        elif kind == "ref":
        ...
        if sym == "lparen":
            ...
            while True:
                ...
                if table[i].params[p] == True:
                    ...
                    if sym == "ident":
                        if table[i].kind == "var" or table[i].kind == "val":
            ...
            if sym != "rparen":
                error(22)
            gen("INT", 0, -(3+p))
        # ----------------- Statement Call Modifications End
        ...
        while True:
            statement(tx, level)
            # Until sym marks the end of the repeat loop
            if sym == "UNTIL":
                break
            if sym != "semicolon":
                error(5)
        ...
        generalexpression(tx, level)
        gen("JPC", 0, cx1)
    # Repeat Until Modifications End
        ...
        expression(tx, level)
        gen("STO", level-table[i].level, table[i].adr)
        # ensures that either TO or DOWNTO must be present
        if sym != "TO" and sym != "DOWNTO":
            error(29)
        symbol = sym # storing current symbol to be used later
        getsym()
        expression(tx, level)
        cx1 = codeIndx
        gen("CTS", 0, 0)
        ...
        # calls appropriate opr based on the stored symbol
        if symbol == "TO":
            gen("OPR", 0, 11)
        elif symbol == "DOWNTO":
            gen("OPR", 0, 13)
        ...
        # generates p-code to increment (or decrement) the var by one
        gen("LOD", level-table[i].level, table[i].adr)
        gen("LIT", 0, 1)
        # increments if to, decrements if downto
        if symbol == "TO":
            gen("OPR", 0, 2)
        elif symbol == "DOWNTO":
            gen("OPR", 0, 3)
    ...
    # For modifications end
    
#-- Case Modifications Begin **************************
    elif sym == "CASE":
        firstcase = True
        getsym()
        expression(tx, level)
        ...
        getsym()
        while True:
            # CEND can skip the loop all together
            if sym == "CEND":
                fixJmp(cx2, codeIndx)
                gen("INT", 0, -1)
                break
            if sym != "number" and sym != "ident":
                error(31)
            # ensures the ident is a constant
            if sym == "ident":
                i = position(tx, id)
                if i==0:
                    error(11)
            ...
            # pushes the proper literal value 
            gen("CTS", 0, 0)
            if sym == "number":
                gen("LIT", 0, num)
            elif sym == "ident":
...
def expression(tx, level):
    ...
    while sym == "plus" or sym == "minus" or sym == "OR": # adds OR 
        addop = sym
        getsym()
        term(tx, level)

        # does appropriate opporation
        if(addop == "minus"):
...
def term(tx, level):
    global sym;
    factor(tx, level)
    while sym=="times" or sym=="slash" or sym=="AND": # adds AND
        mulop = sym
        getsym()
        factor(tx, level)
        if mulop == "slash":
...
def factor(tx, level):
    ...
    if sym == "ident":
        ...
        if table[i].kind == "const":
            gen("LIT", 0, table[i].value)
        #--------------- ident can also be val
        elif table[i].kind == "variable" or table[i].kind == "val":
            gen("LOD", level-table[i].level, table[i].adr)
        #--------------- ident can also be ref
        elif table[i].kind == "ref":
            gen("LDI", level-table[i].level, table[i].adr)
        elif table[i].kind == "procedure" or table[i].kind == "funtion":
            error(21)
        getsym()
    elif sym == "number":
        gen("LIT", 0, num)
        ...
        getsym()
        generalexpression(tx, level) # calls general expression instead
        if sym != "rparen":
            ######
            error(22)
        getsym()
